...
nodes = [nodeFunction.QuasiPeriodic(100, 25, 1, 0.1)]
weight = weightFunction.Constant(0)
weight_values = [np.sqrt(2)*rms_rv**2, np.sqrt(2)*rms_bis**2]
means = [None, None]
...
weight = weightFunction.SquaredExponential(1, 100)
weight_values = [np.sqrt(2)*rms_rv, np.sqrt(2)*rms_bis]
means = [None, None]
