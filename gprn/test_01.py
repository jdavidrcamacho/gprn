...
##### Our GP #####
weight = weightFunction.Constant(10)
...
##### Seting priors #####
#weight function
weight_const = stats.uniform(np.exp(-10), 100 -np.exp(-10)) #from exp(-10) to 100
#node function
node_le = stats.uniform(1.1, np.exp(10) -1.1) #from exp(-10) to 1
node_p = stats.uniform(15, 35-15) #from 15 to 35
node_lp = stats.uniform(np.exp(-10), 10 -np.exp(-10)) #from exp(-10) to exp(10)
node_wn = stats.uniform(np.exp(-10), 0.1 -np.exp(-10)) #from exp(-10) to exp(10)

#weight value
weight_amp = stats.uniform(np.exp(-10), 100 -np.exp(-10)) #from exp(-10) to 100
