...
node_lP = stats.uniform(np.exp(-10), 10 -np.exp(-10))
...
def from_prior():
    return np.array([node_le.rvs(), node_p.rvs(), node_lp.rvs(), node_wn.rvs(),
                     ...
                     weight_3.rvs(), weight_4.rvs()])


##### MCMC properties #####
import emcee
runs, burns = 1000, 1000 #Defining runs and burn-ins
...
def logprob(p):
    if any([p[0] < np.log(10), p[0] > 10, 
        ...
        new_node = [nodeFunction.QuasiPeriodic(np.exp(p[0]), np.exp(p[1]), 
                    ...
                    nodeFunction.Periodic(np.exp(p[4]), np.exp(p[5]), 
                                          np.exp(p[6]))]
...
l1,p1,l2,wn1, lP1,pP1,wnP1, w1,w2,w3,w4 = map(lambda v: (v[1], v[2]-v[1], 
                                                         v[1]-v[0]),
...
#median and quantiles
l1,p1,l2,wn1, lP1,pP1,wnP1, w1,w2,w3,w4, likes = map(lambda v: (v[1], v[2]-v[1], 
                                                                v[1]-v[0]),
                             zip(*np.percentile(samples, [16, 50, 84],axis=0)))
